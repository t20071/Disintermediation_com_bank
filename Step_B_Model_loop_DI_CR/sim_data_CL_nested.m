...
% compute scaled utilities (vectors)
u1_vec = mu_c(1) * (x1 * theta1);  % J1x1
u2_vec = mu_c(2) * (x2 * theta2);  % J2x1

% avoid overflow: subtract max before exponentiating
M1 = max(u1_vec);
M2 = max(u2_vec);

% if M1 or M2 are NaN (bad inputs) bail out
if ~isfinite(M1) || ~isfinite(M2)
    error('Non-finite utility values encountered.')
end

v1 = exp(u1_vec - M1)';    % 1xJ1
v2 = exp(u2_vec - M2)';    % 1xJ2
 
% Level 1: log-sum-exp restored
V_c1 = (log(sum(v1)) + M1) / mu_c(1);
V_c2 = (log(sum(v2)) + M2) / mu_c(2);
