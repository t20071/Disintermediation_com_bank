...
T = 2000;       % periods to simulate
S = 750;        % length of time window anchored at the end of T over which to compute various metrics
nest = 0;       % nested (1) or non-nested (0) conditional logit model for non-banks' choice of money
alphas = [1.5 1.5]; % base utility for cash and CBDC
beta = 120;     % non-banks' price sensitivity
gamma = 1.221;  % annual velocity of money
delta = .75;    % probability from [0,1] for deposit account move to imply loan transfer
lambda = 0.10;  % required reserves in % of deposits, from [0,1] interval
rB = 0.056;     % banks' reserve borrowing rate (p.a.)
rH = [.00 .0335]; % remuneration rates for banks' required and excess reserve holdings (p.a.)
rDC = .00;      % rate on CBDC (p.a.)
capcon = 1;     % cap spending to maximize chances that non-banks will service loan interest (1=YES, 0=NO)
qu = 1;         % scale model to quarterly (1=YES, 0=NO)
dynG = 0;       % find G dynamically (=1), or fix it (=0)
G = 10;         % when dynG==0, set grid length; irrelevant otherwise
M = 290000;     % total money, in currency million/billion/trillion/whatever (no impact on model dynamics)
plots = 1;      % plots on (1) or off (0)
...
if qu==1                  % convert rates and velocity of money from annual to quarterly
    ...
    f = 4;                % for properly scaling all relevant rates throughout the simulation
end
...
iqrs = +Inf; iqrl = NaN;  % initial IQR reference values
count = 0;                % count of periods per G
modi = 0;                 % to signal that G is to be increased during the simulation
stick = 0;                % indicator that will turn 1 once the final G is deemed to be found and stuck to
if dynG==1
    G = 5;
end

%% Flows (see Table 2 in the paper)
...
%% Balance sheets (see Table 1 in the paper)
...
%% T0 balance sheets
BS.NB.L(1,:) = M/N; % Initialize non-banks' liabilities as the total money divided by the number of non-banks
BS.NB.M(1,:) = BS.NB.L(1,:); % Set non-banks' money holdings equal to their liabilities

BS.BA.L(1,:) = sum(repmat(BS.NB.L(1,:),B,1).*(NatB_L(1,:)==(1:B)'),2)'; % Calculate banks' loans as the sum of non-banks' liabilities assigned to each bank
BS.BA.D(1,:) = BS.BA.L(1,:); % Set banks' deposits equal to their loans
BS.BA.R(1,:) = lambda*BS.BA.D(1,:); % Calculate banks' reserves as a fraction (lambda) of their deposits
BS.BA.B(1,:) = lambda*BS.BA.D(1,:); % Calculate banks' reserve borrowings as a fraction (lambda) of their deposits

BS.CB.R(1)   = sum(BS.BA.R(1,:)); % Calculate central bank's reserves as the total reserves held by banks
BS.CB.C(1)   = 0; % Initialize central bank's cash holdings to zero
BS.CB.DC(1)  = 0; % Initialize central bank's CBDC holdings to zero
BS.CB.B(1)   = sum(BS.BA.B(1,:)); % Calculate central bank's borrowings as the total borrowings from banks
...
for t=1:T
    ...
    if ts==1 % TS Learning Part I
        ...
        expl(t) = bb; % record who the explorer is
        [~,g] = min(abs(iG-rates(t,bb))); % position in the rate grid
        if g==1 % lower edge, step up
            pB = 1;
        elseif g==G % upper edge, step down
            pB = 2;
        else % interior, step according to beta draw
            [~,pB] = max([betarnd(mA(1,g),mB(1,g)) betarnd(mA(2,g),mB(2,g))]); % Lines 2-6 in Table 3 in the paper
        end
        if pB==1
            rates(t,bb) = iG(g+1); % step up (line 7 in Table 3)
        else
            rates(t,bb) = iG(g-1); % step down (line 7 in Table 3)
        end
    end
    
    % Step 2: Non-banks choose where to deposit or hold cash/CBDC and from which bank to have their loans
    if t>1
        % deposits (see Annex II in the paper)
    ...
    if ts==1 % alter the beta distribution (line 10 in Table 3 in the paper)
    ...
    % Step 9: Dynamic gridding
    % Note: The dynamic gridding feature can be used to determine the
    % optimal G, i.e., the optimal deposit rate grid resolution. The
    % underlying intuition for the presence of an optimum is that at too
    % low levels of G, the variance of the equilibrium is high; with
    % further rising G, the variance decreases; at some point, however, a
    % too fine G will imply a growing variance again, which is due to the
    % inherent stochasticity and implied "base variance" stemming from
    % non-bank money holding choices. Users can deactivate this feature and
    % just set G to a value (such as 10, 15, 20...).
    % The dynamic gridding feature is not described in the paper. A follow-
    % up paper will explain it. 
    if dynG==1
        iqrconv(t,1:2) = assess_IQR(avrate(t-count(end):t),0.5);
    end
    count(end) = count(end)+1;
    if count(end)>300 && dynG==1 && stick==0
        if abs(1-(iqrconv(t,2)/iqrconv(t,1)))<0.05 || iqrconv(t,1)==0
            if (iqrconv(t,2)*100)<iqrs(end)
                iqrl = [iqrl;iqrconv(t,1)*100]; %#ok<*AGROW>
                iqrs = [iqrs;iqrconv(t,2)*100]; 
                G = G+1;
                modi = 1;
                count = [count;0];
            end
    ...
    if count(end)>1500 && stick==0 && dynG==1 % enter this if at all only once and not again
        if ~isnan(iqrconv(t,2))
            iqrs = [iqrs;iqrconv(t,2)*100]; % the G*+1st's IQR for the record
            iqrl = [iqrl;iqrconv(t,1)*100];
        elseif ~isnan(iqrconv(t-1,2))
            iqrs = [iqrs;iqrconv(t-1,2)*100];
            iqrl = [iqrl;iqrconv(t-1,1)*100];
        elseif ~isnan(iqrconv(t-2,2))
            iqrs = [iqrs;iqrconv(t-2,2)*100];
            iqrl = [iqrl;iqrconv(t-2,1)*100];
        end
        G = G-1; % back to the optimal G*
        modi = 1; % adjust the grid a final time to G*
        stick = 1; % stop dynamically modifying G after that
        count = [count;0]; % but keep counting for the record
    end
end

%% Analytical rates and market shares as a benchmark (see Annex I in the paper)
...
disp(['Nash deposit rate - Simulated (Analytical): ' num2str(median(avrate(end-S+1:end))) ' (' num2str(ii(1)) ')' ])
disp(['Cash/M - Simulated (Analytical): ' num2str(cr) ' (' num2str(ms(B+1)) ')']);
disp(['CBDC/M - Simulated (Analytical): ' num2str(cbdcr) ' (' num2str(ms(B+2)) ')']);
disp(['D/M - Simulated (Analytical): ' num2str(1-cr-cbdcr) ' (' num2str(1-ms(B+1)-ms(B+2)) ')'])
x1 = BS.CB.R./sum(BS.BA.D,2);
disp(['R/D - Simulated: ' num2str(median(x1(end-S+1:end)))])
x2 = BS.CB.R./sum(BS.NB.M,2);
disp(['R/M - Simulated: ' num2str(median(x2(end-S+1:end)))])
if dynG==1
    disp(['G* = ' num2str(G(end))])
    if stick==1
        disp('Convergence with regard to optimal G likely achieved.')
    else
        disp('Convergence with regard to optimal G likely *not* achieved.')
    end
end
clear x1 x2 cr cbdcr